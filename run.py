...
file_list = list_files_recursively(os.path.join(os.path.dirname(__file__), 'input_mhtml_files_here'))
print(file_list)
...
Examples:
  ...
  %(prog)s document.mhtml --no-css --no-images --buffer_size 16384
    ...
    all_success = True  # Thêm biến để kiểm tra trạng thái tổng thể
    ...
    for file in file_list:
        ...
        print(out_dir_name)
        dir = os.path.join(os.path.dirname(__file__), 'output', out_dir_name)
        print(dir)
        ...
        try:
            ...
            if stats.total_parts == 0:
                ...
                all_success = False  # Đánh dấu có lỗi
    ...
    # Sau khi xử lý tất cả file, mới exit
    if all_success:
        exit(0)
    else:
        exit(1)